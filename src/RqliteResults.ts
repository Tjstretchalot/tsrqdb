import { RqliteSQLError } from './errors';
 ...
 * data
 */
export class RqliteResultItemAdapter {
  ...
  get results(): ReadonlyArray<ReadonlyArray<any>> | undefined {
    return this.raw.values ?? undefined;
  }

  get lastInsertId(): number | undefined {
    return this.raw.last_insert_id ?? undefined;
  }

  get rowsAffected(): number | undefined {
    return this.raw.rows_affected ?? undefined;
  }

  get error(): string | undefined {
    return this.raw.error ?? undefined;
  }
}
 ...
 */
export const raiseIfSQLError = (
  result: AdaptedRqliteResultItem | RawRqliteResultItem
): void => {
  if (result.error !== null && result.error !== undefined) {
    throw new RqliteSQLError(
      `A SQL error occurred: ${JSON.stringify(result.error)}`,
      result.error,
      0
    );
  }
 ...
 * implementation.
 */
export class RqliteBulkResultAdapter {
  readonly itemsRaw: RawRqliteResultItem[];
  private adaptedItems: AdaptedRqliteResultItem[] | undefined;

  constructor(raw: RqliteBulkResultRaw) {
    this.itemsRaw = raw.results;
    this.adaptedItems = undefined;
  }

  get items() {
    if (this.adaptedItems === undefined) {
      this.adaptedItems = this.itemsRaw.map(
        (raw) => new RqliteResultItemAdapter(raw)
      );
    }
 ...
 * contain an error.
 */
export const raiseIfAnySQLError = (result: RqliteBulkResult): void => {
  for (let idx = 0; idx < result.itemsRaw.length; idx++) {
    const item = result.itemsRaw[idx];
    if (item.error !== null && item.error !== undefined) {
      throw new RqliteSQLError(
        `A SQL error occurred on query at index ${idx}: ${JSON.stringify(
          item.error
        )}`,
        item.error,
        idx
